...
static uint8_t g_ui8LedState;
static int32_t ui32LEDToggleDelay ;

extern xSemaphoreHandle g_pUARTSemaphore;

//*****************************************************************************
//
// Timer callback for led.
//
//*****************************************************************************
void vTimerCallback(TimerHandle_t xTimer)
{

    int32_t ui32LEDToggleDelayOld = ui32LEDToggleDelay;
    uint8_t i8Message;
    ...
    if(xQueueReceive(g_pLEDQueue, &i8Message, 0) == pdPASS)
    {
        ...
        if(i8Message == RIGHT_BUTTON)
        {
            ...
            if(xTimerChangePeriod(xTimerLed,pdMS_TO_TICKS(ui32LEDToggleDelay),100) == pdPASS)
            {
                //
                // Guard UART from concurrent access. Print the currently
                // blinking frequency.
            ...
            {
                ui32LEDToggleDelay = ui32LEDToggleDelayOld;
            }

        }
    }
    
    if(g_ui8LedState == LED_ON)
    {
        //
        // Turn off the LED.
        //
        RGBDisable();
        g_ui8LedState = LED_OFF;
    }
    else
    {
        //
...
LEDTaskInit(void)
{
    ...
    //
    xTimerLed =  xTimerCreate("LED Timer",
                            pdMS_TO_TICKS(LED_TOGGLE_DELAY),
                            pdTRUE,(void *) 0,
                            vTimerCallback);
    ...
    {
        if(xTimerStart(xTimerLed,0) != pdPASS)
        {
